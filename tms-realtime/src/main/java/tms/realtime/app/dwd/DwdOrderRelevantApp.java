    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<String> orderDetailDS = keyedDS.process(new KeyedProcessFunction<String, JSONObject, String>() {
            ...
            public void open(Configuration parameters) throws Exception {
                ...
                infoBeanStateDescriptor.enableTimeToLive(new StateTtlConfig.Builder(Time.seconds(5)).build());
            ...
            @Override
            public void processElement(JSONObject jsonObject, KeyedProcessFunction<String, JSONObject, String>.Context ctx, Collector<String> collector) throws Exception {
                //获取处理的表名
                String table = jsonObject.getString("table");
                //获取对表的操作类型
                ...
                if ("order_info".equals(table)) {
                    ...
                    if ("c".equals(op)) {
                        ...
                        if (detailOriginBean == null) {
                            //订单数据比订单明细数据先到，将订单数据放到状态中
                            infoBeanState.update(infoOriginBean);
                        } else {
                            //说明订单数据来之前，明细数据已经到了，将订单和明细数据进行关联
                            //属于下单业务过程
                            ...
                            //将下单业务数据放到主流中
                            collector.collect(JSON.toJSONString(dwdTradeOrderDetailBean));
                        }
                    } else if ("u".equals(op) && detailOriginBean != null) {
                        //下单外的其它操作
                        //获取修改前的数据
                        JSONObject oldData = jsonObject.getJSONObject("before");
                        //获取修改前状态的值
